...
public class JavaSqlExample1 {
	public static void main(String[] args) {
		...
		// 게시글 조회 쿼리(query)
		String sql1 = "SELECT seq, name, content, create_date, price FROM board";
		...
		try {
			...
			while(rs.next()) {      // rs,next()는 가져온 데이터들의 행을 가져오고 값이 있다면 true를 변환
				...
				// 각 행의 seq컬럼의 행을 가져온다
				System.out.println("게시글 제목:" +  rs.getString("name"));          
				...
				System.out.println("게시글 조회수:" +  rs.getString("price"));    
